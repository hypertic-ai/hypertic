    ...
    def interpret_ratio(self, ratio_name: str, value: float) -> str:
        ...
        interpretations = {
            "current_ratio": lambda v: (
                "Strong liquidity" if v > 2 else "Adequate liquidity" if v > 1.5 else "Potential liquidity concerns" if v > 1 else "Liquidity issues"
            ),
            "debt_to_equity": lambda v: (
                "Low leverage" if v < 0.5 else "Moderate leverage" if v < 1 else "High leverage" if v < 2 else "Very high leverage"
            ),
            "roe": lambda v: (
                "Excellent returns"
                if v > 0.20
